...
from app.models import QualityCheck, SaleRecord, InventoryItem, ProofingTelemetry, QualityInspection
from sqlalchemy.orm import Session
import random

products = [
    ('Butter Croissant', 85.0), ('Sourdough Loaf (400g)', 120.0),
    ('Almond Danish', 65.0), ('Chocolate Eclair', 55.0),
    ('Butter Cookies (box of 12)', 180.0), ('Multigrain Bread Loaf', 95.0),
    ('Cinnamon Roll', 45.0), ('Pain au Chocolat', 75.0),
    ('Focaccia Slice', 40.0), ('Muffin (Chocolate)', 38.0),
]
...
with Session(engine) as s:
    ...
    today_rev = sum(
        float(r.total_amount) for r in s.query(SaleRecord).filter(
