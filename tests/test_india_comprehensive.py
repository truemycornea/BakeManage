...
def _post(path: str, json: dict | None = None, headers: dict | None = None, **kwargs):
    ...
    return requests.post(
        f"{BASE_URL}{path}", json=json, headers=headers or HEADERS, **kwargs
    )
...
def _patch(path: str, params: dict | None = None, headers: dict | None = None):
    ...
    return requests.patch(
        f"{BASE_URL}{path}", params=params, headers=headers or HEADERS
    )
...
class TestAuthentication:
    ...
    def test_login_wrong_pin_returns_401(self):
        r = _post(
            "/auth/login",
            json={"username": "admin@bakemanage.io", "pin": "totally-wrong-xyz"},
        )
        assert r.status_code == 401

    def test_login_unknown_user_returns_401(self):
        r = _post(
            "/auth/login", json={"username": "nobody@bakemanage.io", "pin": _ADMIN_PIN}
        )
        assert r.status_code == 401
...
class TestDocumentIngestion:
    ...
    def test_ingest_image_invoice(self):
        ...
        with open(fp, "rb") as f:
            r = _post(
                "/ingest/image",
                files={"file": ("indian_vendor_invoice.png", f, "image/png")},
                json=None,
                headers=HEADERS,
            )
        assert r.status_code == 200, r.text
    ...
    def test_ingest_recipe_image(self):
        ...
        with open(fp, "rb") as f:
            r = _post(
                "/ingest/image",
                files={"file": ("indian_recipe_barfi.png", f, "image/png")},
                json=None,
                headers=HEADERS,
            )
        assert r.status_code == 200, r.text
...
class TestQualityControl:
    ...
    def test_browning_endpoint_with_qr_image(self):
        ...
        login = _post(
            "/auth/login", json={"username": _ADMIN_USERNAME, "pin": _ADMIN_PIN}
        )
    ...
    def test_create_kaju_barfi_batch(self):
        r = _post(
            ...
            json={
                ...
                "ingredients": [
                    {
                        "ingredient_name": "Kaju (Cashew) Grade-A",
                        "quantity_used": 3.0,
                        "unit_of_measure": "kg",
                        "lot_number": "KAJU-LOT-042",
                    },
                    {
                        "ingredient_name": "Refined Sugar",
                        "quantity_used": 1.8,
                        "unit_of_measure": "kg",
                        "lot_number": "SUG-LOT-018",
                    },
                    {
                        "ingredient_name": "Desi Ghee",
                        "quantity_used": 0.12,
                        "unit_of_measure": "kg",
                        "lot_number": "GHE-LOT-007",
                    },
                    {
                        "ingredient_name": "Elaichi Powder",
                        "quantity_used": 0.024,
                        "unit_of_measure": "kg",
                    },
                ],
        ...
        r = _post(
            ...
            json={
                ...
                "ingredients": [
                    ...
                    {
                        "ingredient_name": "Vanaspati Ghee",
                        "quantity_used": 2.0,
                        "unit_of_measure": "kg",
                    },
                ],
...
class TestEmployeePerformanceIndia:
    ...
    def test_log_morning_shift(self):
        ...
        r = _post(
            f"/employees/{TestEmployeePerformanceIndia._emp_id}/shift",
            json={
...
class TestQRTableOrderingIndia:
    ...
    def test_place_order_no_auth_required(self):
        ...
        r = requests.post(
            f"{BASE_URL}/tables/{TestQRTableOrderingIndia._qr_token}/order",
            json={
                "order_items": [
    ...
    def test_update_order_to_preparing(self):
        if (
            not TestQRTableOrderingIndia._table_id
            or not TestQRTableOrderingIndia._order_id
        ):
